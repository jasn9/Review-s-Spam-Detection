...
class GANG:
	...
	def __init__(self, user_product_graph, product_user_graph, user_ground_truth,
		...
		sort_priors = [[], [], []]
		sort_priors[0] = sorted(priors[0].keys(), key=lambda x: int(x))
		sort_priors[1] = sorted(priors[1].keys())
		sort_priors[2] = sorted(priors[2].keys(), key=lambda x: int(x))
		self.pu_dim = max(int(sort_priors[0][-1])+1, int(sort_priors[2][-1])+1)
		...
		for id in range(0, self.pu_dim):
			if id < len(self.p_priors):
				if str(id) in product_user_graph:
					self.diag_pu_matrix[id, id] = len(product_user_graph[str(id)])
				else:
					self.diag_pu_matrix[id, id] = 0
			else:
				if str(id) in user_product_graph:
